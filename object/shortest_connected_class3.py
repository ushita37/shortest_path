...
class Graph:
    ...
    # 辺の文字列を辺のインスタンスに変換
    def searchEdgeInstance(self, eName):
        e = None
        for edge in self.es:
            if edge.en == eName:
                e = edge
    ...
    def findMin(self, previousV, tempLabels):
        ...
        for vertex in self.vs:
            ...
            # 新しく永久ラベルを貼ろうとしている点と起点が隣接していない場合（pvnが初期の空文字のまま）も除外する
            if vertex.lb == tempLabels[0] and vertex.pf == False and vertex.pvn != "":
                vertex.pf = True
...
class Edge:
    ...
    def pasteTemporary(self, nvlIdx, previousV, tempLabels):
        if self.nvl[nvlIdx].pf == True:
            pass
        # まだラベルが貼られていない点(pvnが空文字)に、新たにラベルself.ew + previousV.lbを貼る
        elif self.nvl[nvlIdx].pvn == "":
